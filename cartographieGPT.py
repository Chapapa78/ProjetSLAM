    ...
    with open(os.path.join(chemin_colmap, 'images.txt'), 'r') as file:
        for line in file:
            ...
            try:
                ...
                continue
            image_path = os.path.join(chemin_colmap, 'images', data[2])
    ...
    with open(os.path.join(chemin_colmap, 'points3D.txt'), 'r') as file:
        for line in file:
            ...
            try:
                ...
                continue
            points3D[point_id] = np.array(coordinates)
...
def comparer_image_colmap(image, carte, seuil_distance=10):
    ...
    for point_id, coordinates in carte.items():
        distances = np.linalg.norm(np.array([kp.pt for kp in kp_image]) - coordinates[:2], axis=1)
        if np.min(distances) < seuil_distance:
            correspondances.append((point_id, kp_image[np.argmin(distances)].pt))

    return correspondances


# Charger les données COLMAP
cameras, _, points3D = charger_donnees_colmap(CHEMIN_COLMAP)
# Charger les images à comparer
images = charger_images(CHEMIN_IMAGES)

# Stocker les résultats des correspondances
resultats = []

# Comparer chaque image avec les données COLMAP
for image_data in images:
    ...
    if correspondances:
        resultats.append((filename, True))
    else:
        resultats.append((filename, False))

    # Dessiner les correspondances sur l'image
    image_with_correspondances = cv2.drawKeypoints(image, [cv2.KeyPoint(x=pt[1][0], y=pt[1][1], _size=10) for pt in
                                                           correspondances], None, color=(0, 255, 0), flags=0)
    cv2.imshow("Image avec correspondances COLMAP", image_with_correspondances)

    # Attendre une touche pour fermer l'image ou quitter
    key = cv2.waitKey(0)
...
for filename, correspondance_trouvee in resultats:
    if correspondance_trouvee:
        print(f"Correspondances trouvées pour l'image {filename}.")
    else:
        print(f"Aucune correspondance trouvée pour l'image {filename}.")
